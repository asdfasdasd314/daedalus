...
## Summary
The Git Sync panel submits user-scoped commit and pull/push requests to the local daemon, which runs explicit Git command arrays and returns their ordered output through Supabase.

## Key Points
- **Two Explicit Operations**: Commit stages and creates a local commit; Sync pulls then pushes already committed work.
