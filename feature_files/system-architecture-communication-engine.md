...
- **Feature-File Hints Only**: Architecture prompts pass only `feature_files/` path hints and relevant feature-file lists; agents may still query Graphify themselves for structural relationships.
...
- `local-daemon/src/daedalus_daemon/architecture.py`: Detached snapshot creation, feature-file hint assembly, model invocation, validator handoff, and asynchronous generation lifecycle.
...
- 2026-07-17: Prepended TASK_MODE architecture on architecture generation prompts so AGENTS.md routes to the architecture profile.
- 2026-07-17: Restricted architecture prompt hints to feature_files paths only, dropping full changed-file lists and precomputed Graphify community dumps.
