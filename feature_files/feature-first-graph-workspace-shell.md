...
## Summary
The feature-first graph workspace shell owns the responsive workspace framing around the feature graph in the Next.js frontend. It keeps the graph mounted as the background canvas, routes the ventures drawer and primary overlays, preserves the top messages/status HUD, and reuses the shared agent session UI for both new-feature creation and node-focused feature editing.
...
- **Workspace Utilities**: A compact refresh control, session identity, and sign-out live in a utility area instead of a first-class settings panel.
