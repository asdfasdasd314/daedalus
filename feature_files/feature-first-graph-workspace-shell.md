...
## Summary
The feature-first graph workspace shell owns the responsive workspace framing around the feature graph, including primary overlays, Ventures, the prompt-only Edit surface, the Agent Output Viewer, and global routing between Feature View, Architecture View, and AOP Beta.
...
- **Global Workspace Mode**: A fixed top-center toggle routes Feature View, Architecture View, and AOP Beta. Non-feature views leave the graph mounted and inert so pan, physics, layout, selection, and zoom state survive; Architecture mounts the visualization canvas, AOP Beta mounts the bridge-only session surface.
...
- 2026-08-08: Extended the top-center toggle with AOP Beta workspace view (bridge-only session surface) alongside Feature View and Architecture View.
