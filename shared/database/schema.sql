...
-- Migration 050: repair stale AOP execution state and preserve loop ownership on task resume.
update aop_execution_loops loop_row set
  status='awaiting_start',current_agent_task_id=null,active_prompt_id='',pause_requested=false,cancel_requested=false,
  status_detail=case task.status when'blocked'then'Coding task is blocked. Complete the required operator action, then Resume task from Agent Output History.'else'Coding task failed. Review it, then Resume task from Agent Output History.'end,
  updated_at=now()
from agent_tasks task
where loop_row.current_agent_task_id=task.id and loop_row.status='executing'and task.status in('blocked','failed');
...
returns jsonb language sql stable security definer set search_path=public as $$
 ...
 from (select 1) seed left join agent_tasks t on t.id=p_task_id and t.user_id=p_user_id;
$$;
revoke all on function daemon_get_agent_task_control(uuid,uuid) from public;
grant execute on function daemon_get_agent_task_control(uuid,uuid) to anon;
...
begin
 update agent_tasks set
   ...
   message='daemon_review',cancel_requested=false,completed_at=null,retry_generation=retry_generation+1,
   resolver_attempts=case when status='blocked'then 0 else resolver_attempts end,updated_at=now()
 where id=p_task_id and user_id=auth.uid()and updated_at=p_expected_updated_at and status in('failed','blocked')and worktree_path is not null and branch_name is not null
 returning source_loop_id into resumed_loop_id;
 if not found then return false; end if;
 if resumed_loop_id is not null then
   update aop_execution_loops set status='executing',paused_from='',current_agent_task_id=p_task_id,active_prompt_id=p_task_id::text,
     pause_requested=false,cancel_requested=false,status_detail='Resuming the retained coding task; revalidating the operator prerequisite.',updated_at=now()
   where id=resumed_loop_id and user_id=auth.uid()and status not in('completed','cancelled','failed');
 end if;
 return true;
end;
$$;
