...
export default function EntryPointPicker({ acceptsWork, featureMarkdown, parameterFile, pending, daemonError, onSave }: Props) {
  ...
  return <section className="grid gap-3 rounded-[1.25rem] border border-white/10 bg-slate-900/70 px-4 py-4">
    ...
    <label className="grid gap-2"><span className="text-xs text-slate-400">Project-relative path</span><input value={draft} disabled={pending || !acceptsWork} onChange={(event) => setDraft(event.target.value)} className="rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm text-slate-100 disabled:opacity-50" placeholder="src/example.py" /></label>
    ...
    <div className="flex justify-end"><button type="button" disabled={pending || !acceptsWork} onClick={() => void submit()} className="rounded-full bg-cyan-300 px-4 py-2 text-xs font-semibold text-slate-950 disabled:opacity-50">{pending ? "Saving..." : operation === "add" ? "Add entry point" : operation === "update" ? "Update entry point" : "Remove entry point"}</button></div>
  </section>;
}
